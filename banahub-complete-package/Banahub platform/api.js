...
window.api = {
  ...
  getPrograms:       ()        => apiFetch('/api/programs').then(r=>r.json()),
  getSponsorshipPackages: ()   => apiFetch('/api/sponsorship-packages').then(r=>r.json()),

  // Stripe Checkout — redirects to Stripe-hosted page. Card data never
  // touches this site. Price is looked up server-side; nothing here can
  // change the amount charged.
  checkout: async function(itemType, itemId) {
    const token = (await window.getBanaAccessToken()) || localStorage.getItem('bana_access_token');
    const user = window.api.currentUser();
    const res = await fetch(CONFIG.SUPABASE_URL + '/functions/v1/create-checkout-session', {
      method: 'POST',
      headers: Object.assign(
        { 'Content-Type': 'application/json' },
        token ? { 'Authorization': 'Bearer ' + token } : {}
      ),
      body: JSON.stringify({ item_type: itemType, item_id: itemId, email: user && user.email }),
    });
