...
-- Storage bucket for admin-uploaded files (images, PDFs, docs). Public
-- read (so uploaded URLs work directly on the site), admin-only write
-- via the app (enforced by the /admin/content/media route + media_admin_all
-- policy above — the bucket policy below governs direct storage access).
