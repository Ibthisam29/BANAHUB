...
window.fetch = async function(input, opts) {
  ...
  try {
    ...
    // ── Admin: pages ─────────────────────────────────────────────────────────
    if (route.startsWith('/admin/content/pages') && method === 'GET') {
      const { data } = await sb.from('site_pages').select('*').order('slug');
      return _resp({ pages: data || [] });
    }
    if (route.startsWith('/admin/content/pages/') && (method === 'PATCH' || method === 'PUT')) {
      const slug = route.split('/admin/content/pages/')[1];
      const { data: { user } } = await sb.auth.getUser();
      const { data } = await sb.from('site_pages').upsert(Object.assign({}, body, { slug, updated_by: user && user.id, updated_at: new Date().toISOString() }), { onConflict: 'slug' }).select().single();
